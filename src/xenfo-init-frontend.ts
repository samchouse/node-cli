...
const initFrontend = () => {
  ...
  const child = spawnSync(
    `git clone -q https://github.com/Xenfo/frontend-template.git frontend && rm -rf frontend/.git/`,
    { cwd, shell: true }
  );
  ...
  if (child.error) {
    ...
    const dependantChild = spawnSync(
      `cd frontend && cp .env.example .env.development && cp .env.example .env.production && yarn`,
      { cwd, shell: true }
    );
...
program.action(() => initFrontend()).parse(process.argv);
