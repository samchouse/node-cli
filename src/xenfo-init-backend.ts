...
const initBackend = () => {
  ...
  const child = spawnSync(
    `git clone -q https://github.com/Xenfo/backend-template.git backend && rm -rf backend/.git/`,
    { cwd, shell: true }
  );
  ...
  if (child.error) {
    ...
    const dependantChild = spawnSync(
      `cd backend && cp .env.example .env.development && cp .env.example .env.production && yarn`,
      { cwd, shell: true }
    );
